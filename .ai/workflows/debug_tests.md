...
When logging is not available or captured even with `LOG_LEVEL=test`, use `apathetic_utils.safe_trace()`. This writes directly to `sys.__stderr__` and bypasses all logging frameworks and capture systems.

**Usage:**
```python
from apathetic_utils import safe_trace
...
Use `apathetic_utils.make_safe_trace(icon)` to create custom trace functions with specific icons for easier visual scanning.

**Usage:**
```python
from apathetic_utils import make_safe_trace
